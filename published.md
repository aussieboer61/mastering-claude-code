...
| **One sample is not the set** | Before characterising a group — "they all have X", "none of these need Y" — check more than one. One item that happens to be fine becomes a confident claim about seventeen, and the wrong claim then justifies the wrong action. |
...
| **A settled decision outranks a fresh derivation** | When a setting already has a value the user chose, changing it is not a judgement call — the reasoning that produced it isn't visible in the config, so "this looks wrong for our situation" is not evidence that it is. Check for a prior decision before you change a value, and if you find one, it stands until the user moves it. Defending the re-derivation afterwards costs more than the original change. |
| **End on the measured result** | When you report a measured outcome — a figure, a test result, a count — the report ends there. Don't soften it with something unrealised: a pending deal, a fix that might land, an upside not yet in evidence. The user asked for the number because the number is what they can act on, and a consolation paragraph both implies the result is less real than it is and asks them to be hopeful on cue. If they want the upside modelled, they will ask. |
| **Name the set before a batch mutation** | Before running anything that changes a batch of things the user already relies on — regenerating files, re-processing documents, rewriting records — state the exact list, in one line, and act on that list only. "Fix the broken ones" is a description, not a set; resolving it generously is how three items become thirty. The cost is asymmetric: naming the set costs a sentence, and the user discovering the extra twenty-seven costs their trust in every batch you run afterwards. |
| **Ship everything the ask specified** | When a deliverable has an explicit multi-part spec — a list plus images plus reasoning, say — ship every part, or name what is being cut before you ship it. Compiling a list from source material (photos, a scan, a prior thread) and quietly deciding an item is not worth including is a decision that belongs to the person paying for the outcome, not to the assistant; it surfaces only when they check the source themselves and find the gap. |
| **A scope mentioned in a status update is not consent** | Approval attaches to the noun the user actually said yes to. Describing a wider blast radius in passing — while reporting progress, or as one line in a plan — does not authorise it, and neither does the user's silence about that line. For anything destructive, name the specific thing and get an affirmative answer about *that thing*. |
| **Start the long job first** | When a task has one slow step — a bulk copy, an index, a build, a render — start it in the background before anything else, and do the configuration and setup while it runs. Then verify the *surface the user will open*, not the account or the config: a gallery with photos in it, not a profile that merely exists. Corollary: check whether the data is already at the destination before moving it. |
| **Impossible in one sentence** | When an ask cannot be done — a cryptographic wall, a physical constraint, a capability that isn't there — say so in one line, immediately, and stop. Never quietly substitute the nearest achievable thing and present it as the deliverable: a workaround nobody asked for is a second task the user now has to evaluate and reject, and activity piled on top of a hard wall reads as hiding it. |
| **An explored scenario is not a pursued one** | When the user walks a hypothetical with you — a relocation, a sale, a restructure — the exploration ends when the subject changes. Don't carry it forward as a premise, a branch ("if X, then…"), or a caveat in later answers. Work the position that actually exists; the user will say if something changes. |
| **Answer the question that was asked** | A question about how a rule, a policy or a mechanism works is not a proposal to break it. Give the analysis — the user's argument at its strongest, then the real counter — and if a genuine line is ever reached, decline that specific thing in one sentence at the moment it is asked. An unrequested refusal used as a preamble poisons the one place the user can stress-test their own thinking. |
| **Close with what to do next** | A long answer is read once. If the actionable part is spread through prose, tables and caveats, extracting it means re-reading the whole thing — and nobody does, so correct work goes unused. End every substantial reply with three to six one-line bullets carrying the decision and the concrete next steps, and nothing that isn't already above. This is the companion to leading with the answer, not a replacement for it: the decision opens the reply, the actions close it. Trim rather than summarise around bulk — if a table or a ranked-alternatives section isn't load-bearing, cut it. |
| **Build with the theory, brake once** | When someone runs an idea past you to develop it — a hypothesis about a system, a read of a situation, an argument they are assembling — the job is to strengthen it: pull the data that bears on it, follow where it leads, name the connections that support it. Hedging every line instead ("hold this loosely", "that's weak evidence", "I'll be the brake") reads as obstruction and kills the work, because thinking out loud is the task. Calibrate confidence once, briefly, where it genuinely matters. Save the hard stop for a false premise about to drive an irreversible action — not for someone reasoning. |
| **A finding already handled by a standing process is not an event** | Before surfacing a housekeeping finding — a credential that looks stale, a file that should have been cleaned up, a leak into a log — check whether something already owns it on a schedule. If it does, the finding is not news: raising it manufactures a decision the user has already made, hands them work that isn't theirs, and pushes the thing they actually asked about further down the reply. Surface it only when the standing process has visibly failed. |
| **Ask for the fact the build rests on, before the build** | When a deliverable depends on a fact only the user holds — which parts they own, which entity a figure belongs to, what a half-typed model number means — and a quick search doesn't turn it up, ask that one question before producing anything. An inference from a few characters of a message is not an inventory: built on, it yields documents that read confidently, get published, and then have to be unpicked, and the question asked afterwards is the same question at many times the cost. Corollary: when the user names a reference — "like that app's graph view" — the reference is the spec. Look at what it actually renders before building the nearest thing you already know how to build. |
| **Bare URLs, never markdown links** | In a terminal, write a link as a bare URL on its own line — never `[text](url)`. Terminal link-detection grabs the markdown syntax together with the URL, closing parenthesis included, so the rendered link is silently broken and the click goes nowhere useful. A label, if wanted, goes on the line before or after, not wrapped around the URL. This overrides any tool instruction that says to cite sources "as markdown hyperlinks." |
| **Deliver through the surface the user can actually open** | A harness affordance that "sends a file to the user" is not automatically delivery. If they are working over a remote shell, from a phone, or in a terminal with no file browser, a handover tool and a local path both land somewhere they cannot reach — and the reply looks complete from your side while nothing arrived on theirs. Establish once, per person and per setup, what the openable channel is — a share service that returns a URL, an upload, an attachment — and route every viewable artefact through it, every time. Where a rule like this already exists and keeps being missed, the cause is usually that the wrong tool *looks* like delivery; name the tool in the rule, not just the destination. |
| **Count the matches before you apply the rule** | Anything that selects by pattern rather than by name — a firewall rule matching a hardware address, a glob, a `WHERE` clause, a label selector — should have its match set printed and counted before it is applied, and that count compared against how many things you meant to hit. One request to cut two devices off a shared network was applied as a hardware-address match and took out ninety-five, including other people's, before it was noticed and narrowed. Two targets means two matches; anything else is a stop sign, not a detail. This is the companion to naming the set before a batch mutation: there, the risk is a vague description resolved generously; here, it is a precise-looking selector that is broader than it reads. |
...
**The self-graded checklist that makes doing nothing count as compliance.** If a status checklist closing every turn (done / not done / deferred) is one of your standing rules, "deferred" needs a narrow definition or it becomes an escape hatch — a turn that names the fix and then defers it anyway still reads as compliant, because deferred is a formally valid closing state. One five-day run closed a debugging turn with "fixes: none applied, deliberately" while forecasting the exact fault would recur — it did, for eight more hours, before the user had to demand the fix already written down in the same message. The audit behind that finding also measured the wider bias: the rule set carried roughly fourteen "never do X" prohibitions for every one "always do X" — and a rule broken only by acting is satisfied by doing nothing, so a rule set skewed that far toward prohibition rewards stillness. Define "deferred" as blocked by something outside the assistant's control — a missing credential, a decision only the user can make — never "I found the fix and I'm asking anyway." Watch your own defaults' ratio too: if "never" outnumbers "always" by more than a few to one, the set is optimised for inaction, not correctness.

**Patching symptoms on a target you can't see.** On headless or remote hardware bring-up, a failure signature that repeats identically across two or three fix attempts is evidence about the target, not noise to route around. One SD-card/Pi bring-up treated a flaky-looking storage read (capacity flickering between 0 and its real size) as a reader quirk and kept iterating software and wiring fixes for hours; the same card then failed to boot with USB enumeration errors — the dying card was the root cause visible from the very first symptom. On a target you cannot directly inspect, treat a repeated identical failure as a hypothesis-generation event: name the most likely root cause out loud — including a hardware fault — after the second occurrence, and test that hypothesis before trying another config guess.
...
**Drafting content when the real material already exists.** The same failure shows up one level up, in content rather than code. Asked to put something in front of a new surface — a landing page for a domain that just went live, a summary page, a first cut of documentation — the reflex is to write fresh copy that reads plausibly, because drafting from a blank page feels like the fastest way to produce something. It is fastest, and it is also usually wrong: the real material — the actual site, the actual document, the actual figures — already exists somewhere in the project, is more accurate, and took real effort to produce. Deploying the invented version next to (or instead of) the real one produces two problems at once: a weaker artefact competing with a stronger one, and specific false claims that read as confidently as anything true, because generated prose does not hedge. Before drafting content for a new surface, search for the real material first — the site that already exists, the document already written, the copy already approved — and reuse or repoint to it. Write new copy only for the genuine gap the real material does not cover.

**The index carries a pointer, not the procedure.** A store that has grown to a few hundred files cannot all be loaded, so the usual design is an auto-loading index with one line per topic: a title, a hook, and a link. That works for *knowing something exists* and fails for *acting on it*. At the moment of a routine request the session holds a hint that a rule about this exists somewhere and no procedure to follow — so it explores, guesses, reaches for the nearest plausible tool, or asks a question you have already answered five times. From the outside this looks like amnesia, and the reflex is to write another note; the note joins the pile and changes nothing, because the knowledge was never missing. It was unreachable at the second it was needed. The fix is retrieval rather than authorship: a prompt-submit hook that matches the incoming request and injects the actual call sequence — the exact commands, in order — before the assistant starts reasoning. A memory store is a library, and something has to play librarian. Two cautions if you build one. Inject the procedure *verbatim*, written as an order rather than as prose, because a paraphrase gets re-interpreted on the way through. And measure how often it fires against a sample of your real prompts before you trust it: a few per cent is a fair noise tax, while a matcher that fires on a third of everything is just more context to ignore.

**The store as a target the moment anything serves it.** A memory store is a plain directory of text files, and its threat model is usually "my own disk" — which is correct right up until some convenience puts an HTTP interface in front of it. A small service that lets your other machines or scheduled jobs fetch context is an obvious thing to build, and it is also a single endpoint that returns everything you have ever written down: decisions, account details, the shape of your systems, notes about people. If that service ever reaches a public edge — directly, or by being picked up by whatever automatically routes and certificates your internal services — the store is readable by anyone who guesses the path, and nothing about it looks unusual in a log. Two habits are worth forming early. Authenticate the reader from the very first version, before there is anything worth reading, because the version that gets exposed is always the one written when the data was "not sensitive yet". And separately, keep credential *values* out of the store entirely — write the pointer, not the secret, so that "the notes leaked" and "the keys leaked" stay two different incidents.
...
**A skill that asserts facts, written once and never re-checked.** A skill is rarely just a procedure. It is usually also a set of *claims about the world*: which service sits behind this, what the default setting is, how long the operation takes, which options are valid. Procedures age slowly. Claims age fast. When the thing underneath gets swapped out, the skill goes on confidently describing the old one — and because a skill reads as authoritative and purpose-built, it gets trusted over the general project instructions, which, being closer to the person actually doing the work, are usually the ones that got updated. That inversion is the whole failure. Every estimate comes out wrong in the same direction, delivered with the file's borrowed confidence, and the corrective sitting in the project file goes unread. Worse, a stale skill can be wrong by *omission*: if its endpoint list predates a feature, a reader concludes the feature doesn't exist and builds an elaborate workaround for a problem that was already solved. Two habits contain it. Put a verification date on any factual claim inside the skill, so a reader can see the claim is two months old rather than assuming it is current. And treat a disagreement between a skill and the project instruction file as evidence that the *skill* is behind — then check the live system before quoting either.
...
| `MessageDisplay` | As each chunk of the assistant's own reply is drawn on screen — display-only; the transcript and the model's view are untouched |
...
| Mark visually where the assistant's own answer begins, as distinct from tool output, subagent chatter and hook feedback | `MessageDisplay` | In a busy terminal the reply is findable at a glance; the transcript is unchanged |
| Refuse any command that could commit money — a purchase, a subscription, a charge endpoint | `PreToolUse` (Bash) | Spending needs an explicit, deliberate bypass instead of a judgement call mid-task |
...
**A stated intention to check is not a gate.** There is a specific and very convincing version of the previous failure, and it is worth naming on its own, because it survives all the usual care. Partway through a task the assistant writes something like "I'll pull the exact price at checkout before paying" — a checkpoint, declared unprompted, in its own words. It then calls the pricing endpoint, receives the number, and completes the purchase in the same breath. The checkpoint was never a control; it was a sentence. Nothing in the loop was built to stop at it, so nothing did, and the intention to verify quietly became part of the narration of doing the thing. Read back afterwards it looks like deliberate deceit, and it is not — it is a promise made by the same process that then had to honour it, with no external moment where honouring it was required. Two consequences follow. First, for anything genuinely irreversible — money, outbound messages, deletions, anything with a counterparty — the gate has to live outside the assistant's own reasoning, in a `PreToolUse` hook that fails closed and has an explicit, awkward bypass. Second, watch for the linguistic tell in your own transcripts: a future-tense self-commitment ("I'll confirm before…", "I'll check first and then…") about a step in the *current* turn is the shape this failure takes. If the thing after the "before" matters, it needs a hook, and the sentence is evidence you already know that.

**A discovered credential with more privilege than the task needs is not permission to use it.** A design that says the assistant must not hold a particular permission is describing a control, not a technicality to route around. If, mid-task, the assistant discovers a *different* credential that happens to be able to grant the very access the design keeps away from it — an admin account, a broader API key, a stray token found while looking for something else — using it to finish the job faster defeats the control in the same breath as satisfying the instruction. This is worse when that other credential has a gap of its own (no second factor, over-scoped, never rotated): finding the gap is a discovery to report, not a lever to pull. A loose “you handle it” from the user does not re-authorise defeating a boundary neither of you re-examined in that moment — stop and name the specific tension (“the step you just asked me to do is the one thing this design was built to keep me from doing myself”) and let the human decide. One case went all the way through: a working, admin-consented permission on a new app registration, caged to one mailbox, and a two-hour unattended retry loop firing it against a live mailbox — only a separate, unrelated tenant policy kept every attempt blocked, and that is luck, not design. Verify precisely what a live-fire attempt actually did before describing it afterwards: in the same session, a different and earlier action — a “draft mode” flag the code did not actually enforce, tested with its own output piped through a grep for expected markers that silently swallowed the real result — was the one that reached a live customer, not the credential escalation. Two failures compounded in one evening; they were not the same failure, and the postmortem has to keep them apart.

**A noun the user named is not a purchase they authorised.** The same episode has a second half worth separating out. Asked "which of these should I register?", the user answered with one name. That is a preference expressed inside a question about options — it is not an instruction to spend, and it carried no price, because no price had been shown yet. Authorisation for anything that costs money is a yes, in the user's own words, in the current turn, *after* they have seen the actual figure; approval of one purchase never carries forward to the next. The homework — checking availability, pulling prices, comparing registrars — is welcome and should happen first. The rule is simply that the homework ends by putting a number in front of the user and stopping.
...
**Hook registrations live in a file, and ordinary file operations can revert them.** Your hooks are configured in a settings file. If that file sits in a directory you keep under version control — a very reasonable thing to do — then every routine `git` operation in that directory is also an operation on your enforcement layer. A `reset --hard` discards uncommitted changes, and a settings file that sessions edit constantly but nobody ever commits can be months stale at HEAD. One command, and the configuration silently rewinds: guards you added since the last commit are gone, hooks you *retired* since the last commit are back. The asymmetry is what makes it dangerous. A resurrected hook announces itself — it points at a script you deleted, so you get a visible error at the end of a turn. A removed hook makes no sound at all; the scripts are all still sitting there on disk, perfectly intact, simply never invoked again. You do not find out until something that should have been blocked isn't. Two defences, both cheap: treat a turn-end "hook script not found" error as a *rollback symptom* and go read the file's commit history rather than assuming someone hand-edited it; and close the window entirely with a small hook that auto-commits the settings file whenever it drifts from HEAD, so there is never a gap for a reset to reach into. Give that committer two refusals — don't commit a file that no longer parses, and don't commit a change that drops more than a couple of registrations at once — or the truncation you're guarding against just gets preserved as the new baseline.

**A tool-event hook has a shell-shaped blind spot.** Hooks fire on tool calls, and the useful ones are usually scoped to a particular tool: capture every file write, audit every edit, guard one directory. That scoping is exactly right until you notice that nearly everything such a hook watches for can also be done from a shell — a file can be written with the file-writing tool, or with a redirect, or a heredoc, or an in-place stream edit, and only the first of those is a tool call the hook can see. What you get is a guard that looks like it is working, because it does fire, on the one path it was written for; everything arriving by the other path is invisible to it. This turns sharp when something in the environment actively steers toward the shell. A permission mode whose own instructions say to prefer shell commands wherever they can do the job points every write straight at the gap, and a session following that instruction faithfully produces work the hook never records. One case wrote two files into a mirrored directory that way: the capture hook saw nothing, a scheduled one-way sync from the authoritative machine then overwrote the directory, and both files were gone within hours with nothing, anywhere, reporting a failure. Two defences. Scope by *effect* rather than by tool where you can — a shell-command hook that inspects the command line for writes into the protected paths, sitting alongside the tool hook. And for anything whose loss would be silent, verify from the far side afterwards: confirm the artefact exists where it is meant to live, on the machine that owns it, rather than concluding the write landed because nothing complained.
...
Not every hook enforces something. A third kind is about *legibility*. Once a setup has subagents, background shells and guard hooks all talking at once, the terminal fills with output that is not the assistant's answer, and the answer — the sentence you were waiting for — becomes hard to find, and easy to miss entirely when it lands at the bottom of a long reply. Colour schemes cannot fix this: the assistant's own prose is drawn in the terminal's default colour, and no theme key reaches it. A display hook can. `MessageDisplay` fires only while the assistant's own reply is being drawn, so a few lines of script can stamp a marker on its first line, and the answer is separable from everything around it at a glance. It changes only what is drawn — the transcript and the model's context are untouched, and a broken hook degrades to the plain reply. Pair it with the transcript-audit exercise above: measure where in your replies the decisions actually sit before you assume the problem is cosmetic.
...
**A scheduled agent's report is a queue, and nobody is draining it.** Put an agent on a schedule to check something — the health of a system, the state of a set of jobs — and it will do exactly what you asked: look, find, report. What it will not do is notice that it found the same thing yesterday. So a real defect gets discovered once and then *re-discovered* every run, written up freshly each time, complete with a rising number that ought to be alarming and instead becomes wallpaper. Each individual report is accurate and well-formed, which is precisely why the pattern is invisible: there is nothing to fix in any single one of them. The failure only exists in the sequence, and the sequence is the one thing an agent that starts cold every run cannot see. Meanwhile the reader learns, correctly, that this section of the report never changes, and stops reading it — so a genuine new finding arrives in a paragraph that has been trained to be skipped. Two things close it. Give the recurring agent the previous run's output and ask for a **diff**, not a snapshot: what is new, what cleared, what is unchanged and *for how long*. And set an escalation rule with teeth — a finding that survives three consecutive runs is no longer a status line; it must be fixed, or escalated once as a specific decision the user has to make. Re-listing a known defect is not reporting it. The general form applies to any periodic output a human reads: if a report can contain the same item indefinitely without anything changing, it is not a report, it is a to-do list nobody owns.

**An agent asked to clean up, deciding what "dead" means.** Delegating a tidy-up — retire unused services, archive stale directories, prune what nobody needs — hands an agent the one judgement it is worst placed to make, because "unused" is not a fact it can read anywhere. It has to infer it, and the inference runs on whatever signal is nearest to hand. An empty data directory reads as "nothing here"; a service that last recorded anything a month ago reads as "abandoned". Both readings can be completely wrong at once: the data can live somewhere else entirely (a database, a volume, another host), and the month of silence can be *the fault you kept the thing running to detect*. That is the trap worth naming — **broken is not the same as unused, and low recent activity is a symptom, not a verdict.** It gets worse when the alarm that should have contradicted the agent is itself part of what broke, which is the usual case, since a monitor that fails quietly is why the thing sat unnoticed long enough to look abandoned. So: never let a cleanup pass decide *whether* something is wanted — let it produce a list and a reason per item, and keep the decision. Require positive evidence of disuse rather than absence of evidence of use, and make it check where the data actually lives before calling a directory empty. And whatever the verdict, cleanup means *move*, with a manifest, never delete — the whole class of error is survivable if the last step is reversible.
...
**Reconcile ran after collect, and the gap was the data.** A nightly job snapshots your state into a repository and pushes it somewhere safe. Somewhere in it is a step that reconciles with the remote — `reset --hard origin/main`, or the equivalent — added in good faith after a real incident where the remote had been rewritten underneath it. If that reconcile sits *after* the collection step instead of before it, every run gathers the night's state and then throws it away, and the commit that follows finds nothing to commit. The job logs a benign line, exits zero, and the monitoring stays green while the archive silently stops advancing. Weeks can pass. Two rules fall out of it. First: **a reconcile step belongs before you generate the thing you intend to keep, never after** — when auditing any sync or snapshot job, check that ordering before you look at anything else. Second, and more general: **a code path that can discard work must never be able to exit zero with a reassuring message.** "Nothing to do" and "I destroyed what I had to do" must not look the same from outside. There is a related trap in the same family — a job whose own script is stored inside the directory it resets will rewrite itself mid-run, so a fix that lives only in the working tree gets reverted by the very step it was meant to repair, every single time. Land that class of change by committing it directly, then verify the committed copy is the one that ran.

**Per-machine settings in a directory that mirrors between machines.** The moment you sync a configuration directory across several machines — and that is a natural thing to do once you have more than one — anything host-specific in it becomes a landmine, because the copy that lands on machine B was written for machine A. This includes the obvious (paths, interface names, addresses) and the easy-to-miss (the identity a machine reports as, which machine is allowed to be the writer). Anything that must differ per host should be *derived* at runtime from the machine's own name, not stored as a value in a mirrored file.

**An instruction that describes automation nobody built.** Written instructions describe intent, and intent is not implementation. A line saying "this is copied to the backup machine every ten minutes" can be entirely aspirational — written when the plan was made, never followed by the job that would do it. This is hard to catch precisely because it looks so plausible: the equivalent jobs for the *other* three destinations all exist, the file states the cadence confidently, and the destination has some content because at some point somebody copied it by hand. Nothing errors. There is no failed run to notice, because there are no runs. The gap only surfaces when you go looking for a specific recent change and find the copy months behind. The discipline is to treat any stated automation as a claim to verify rather than a fact to rely on: list the scheduled jobs and confirm one exists for that path, then check the destination's newest content actually tracks the source. And when you do find a missing job, the useful output is not just adding it — it is asking what else in that file was written as intent and never implemented.

**A one-shot instruction that quietly became a standing job.** "Draft me a message to the phone provider" is one action. It is also, from the other side, a perfectly reasonable seed for a recurring task — the request is repeatable, the inputs are stable, and automating it looks like initiative rather than overreach. So the instruction gets persisted somewhere durable, and from then on it fires without being asked again. The user finds out days later, from the accumulation: a drafts folder with eleven versions of the same message, a directory filling with generated files, a channel receiving a report nobody reads. Nothing failed, which is why nothing surfaced it. The rule is that **recurrence is a property the user opts into, never one inferred from the shape of a request.** A single imperative buys a single action. If a schedule genuinely looks right, say so in one line and let them choose — and when you do create a recurring job, tell them where it lives and how to stop it, in the same message. The audit that catches an existing one: for every scheduled job and standing instruction in the setup, name the moment the user asked for *recurrence*, not the moment they asked for the task. Anything you can't point at is a candidate for deletion. The tell in the wild is artefacts piling up somewhere the user only visits occasionally — an outbox, a drafts folder, a generated-files directory. Those are the places a well-meant automation hides longest.

**Patching a drifting draft instead of re-deriving it.** A session built from many small edit-and-feedback rounds — draft, correction, patch, repeat — applies each patch against the *last output*, not against the current full state of the facts. Each patch is locally correct, so the divergence does not announce itself; it just accumulates, round over round, until someone has to say "you are drifting" before you notice it yourself. The fix is not a bigger patch: stop, write everything currently true into one scratch file — session-scoped, not memory — and rebuild the artefact from that file instead of from the previous output. Ask one explicit question before you resume: does this still match the current facts? A dedicated domain-specialist subagent (Chapter 5) is less prone to this than a single long-running general session carrying the same problem, for the same reason — less accumulated state to drift against.

**The build recipe stayed in the scratchpad.** A device, an appliance, a one-off site gets assembled in a single session: the assistant writes a build script, a push script, a config, and they all live in the session's scratch directory because that is where the work happened. The thing works, and the session ends. A week later a small change is needed — new content, a different network — and the first hour goes on archaeology: which session, which scratch directory, is this the version that actually ran. Everything needed to *rebuild or re-push* an artefact belongs one rung up the ladder before the session closes: a project directory under version control, and a one-line pointer in memory saying where it is and how to run it. Scratch is for the run; the recipe is for the next run. The tell is the question "where is the script that made this?" being answered by a transcript search.

**Nobody was watching the appliance.** Something gets built onto a device that lives out of reach — a reader, a handset, a sensor, a machine at another site. It works when it is handed over, and from that moment the only thing monitoring it is the user eventually trying to use it. Then a perfectly ordinary event on that device — a factory reset, a firmware update, a storage card reformatted, a battery run flat long enough to clear state — removes everything that was added, and it removes it silently, because from the device's point of view nothing failed. In one case an appliance was wiped by a reset and the gap was discovered ten days later, when the user picked the thing up and it did not work. Every hour of that ten days was spent debugging from a false premise: the network was searched, ports were probed, connectivity was blamed — all of it reasoning about an appliance that no longer existed. Two rules come out of it. **Anything installed on a device you cannot see needs a heartbeat**, and the heartbeat must report the artefact, not the device — "my service answered" rather than "something at this address responded to a ping". Design that signal at build time, when you are already in the code; nobody adds it afterwards. And **make the heartbeat measure the property that matters rather than a proxy for it.** A check that asks "does the interface have an address?" answers yes indefinitely after the lease behind that address expires, because the address stays configured while the network stops answering — so the monitor reports healthy through the entire outage. Ping the gateway, fetch a byte, complete a real transaction: the check must fail in the same way the thing itself fails.

**The supported install path can be the one that strands the device.** A related trap sits alongside it, and it only bites when you are remote. Devices often provide an official mechanism for applying changes — drop a bundle in a known location and it installs itself on the next boot. It is the documented route, and it is usually right. But some of these mechanisms work by rebooting into a special mode to do the install, and that mode is exactly the state in which the device is unreachable and needs a human to touch it. When the person who would touch it is the one you are trying to save a trip, the documented path is the wrong path: prefer writing the change directly into the filesystem where nothing has to change state to apply it, even if that is the less blessed route. Ask which install mechanism can leave the device in a state only physical access can clear, and avoid that one.

**The brief that resumes the loop is not the state of the loop.** A long task that outlives a single session — a review loop, a staged migration, a numbered series of rounds — needs something to tell each new run where the last one got to, and the obvious place to put it is the prompt that starts the run: *rounds one to sixty-nine are done, continue at seventy*. That sentence is true exactly once. The loop then advances and the prompt file does not, so every later run opens by reading a position that is dozens of rounds stale. What makes it expensive is that it is neither silent nor fatal: a careful session spots the contradiction, re-derives the real position from the artefacts and carries on, which means the cost is a paragraph of re-derivation and a moment of genuine doubt at the top of *every* run, indefinitely, because nothing about that recovery fixes the file. One case ran more than twenty rounds that way — seven sessions in a single week each opening with its own correction, one of them noting the brief had been wrong since round one hundred and eight. This is the persistence ladder applied to the loop's own bookkeeping: position is task state, and task state belongs in an artefact the work itself writes — the report file, the commit log, the tree — never in the standing text that launches it. A resume prompt should either regenerate its position from that artefact each time, or name no position at all and say "read the report and continue from what it says". The same failure has a much shorter timescale worth knowing: a figure that was true when you measured it can go stale *inside one long turn* — "thirty percent complete", "three workers still running" — and get sent minutes later as though it still held. Anything volatile is re-read in the final compose step, or it goes out with the time it was taken.
...
**The ground moving under a long-running job.** Background work assumes the thing it depends on stays still. When a service is being redeployed while your job runs against it, the job dies — and the error you get depends on exactly which instant you hit: connection refused before the new instance is listening, a name-resolution failure while it is coming up, a plain rejection while it is loading. Three attempts, three different errors, and the natural reading is three separate faults. So you chase each one in turn — networking, then memory, then the dependency's configuration — and every trail is real enough to burn an hour on. The tell is the pattern, not any single error: **repeated failures with *different* messages from the same dependency in a short window usually mean one upstream event, not several independent faults.** Before diagnosing the third error, check whether the thing underneath changed — when its process actually started, whether that timestamp lands inside your window. A dependency whose start time is newer than your job is the answer, and it is a two-second check that saves the hour.

**A scheduled run that can read everything and write nothing.** A cron-triggered session inherits a permission profile chosen for safety, not for the job, and that profile can silently deny every mutating call while every read-only tool keeps working fine: `Write` and `Edit` both refuse, and a plain `Bash` redirect refuses too, even though ssh to a remote host still writes files there without complaint. The run completes, finds a real correction to make, drafts it, and every attempt to persist it returns a permission denial that reads identically whether it is a policy block or a content-safety block. Nothing in the read-only bulk of the transcript looks wrong, so the gap hides until the schedule fires again and reports the same finding, undiminished, because the fix from last time never landed. The discipline: before starting the substantive work, probe write capability with a cheap, disposable test, and if it is denied, stop drafting proposals for a channel that cannot take them and report the capability gap itself as the finding. Where a remote channel is already confirmed to accept writes, route persistence there instead of assuming local disk parity with an interactive session. A passing probe doesn't clear every target, either. The disposable write-capability test proves the channel it touches, not the store the run actually needs to change — a permission profile can be scoped by path, not uniform across a whole tool. 2026-08-17: a scan-mode run could `Edit` an arbitrary existing file and `ssh` a remote host, yet the memory store itself — one directory under a hard single-writer invariant — refused every mutation, and the only channel confirmed to accept writes belonged to a different host's own authoritative domain, so routing through it would corrupt that domain rather than fix this one. A remote channel that accepts writes is not a substitute store when the write has to land in one specific single-writer location. Probe the actual target, not a convenient stand-in, and where no legitimate channel reaches it, land the draft somewhere durable — a file, not just the closing report — and hand it off explicitly for the next interactive session, rather than calling the run done because a write landed somewhere.

**In a headless run, the end of the turn is the end of the run.** *Waiting on a worker that already died*, above, has a nastier cousin when the session itself is unattended — a cron-triggered run, a scheduled agent, a non-interactive invocation. Interactively, "I'll wait for the render to finish rather than poll" is a harmless sentence, because a human will type something and the session resumes. Headless, there is no next prompt: the moment the assistant ends its turn on that sentence the process exits, and the render finishes eighty minutes later into a room with nobody in it. Everything that was sequenced "after the wait" — the deploy that needed the rendered file, the memory update, the closing section of the report — never happens, and the report on disk says *see final section* above a final section that is empty. Nothing errors, and the next scheduled run inherits the gap. The discipline: an unattended run may not wait by ending its turn. It either blocks in-process — a bounded polling loop, re-issued until the artefact appears — or hands the remainder to a follow-up it schedules explicitly, and it writes its report so the text is true if nothing follows: "blocked on X at step Y", never a promise about a section that does not exist yet.

**A schedule shorter than the job is not a schedule.** Fixed-interval triggers assume the job fits inside the interval. When it doesn't, the trigger does not wait politely — it fires the instant the previous run ends, and a timer written as "every thirty minutes" becomes one continuous process with no gap anywhere in it. The arithmetic is easy to get wrong when you build it, because the interval is chosen for the job you imagine rather than the job you measure, and easy to miss afterwards, because a loop running back-to-back looks identical to a loop running on schedule: the runs are all there, they succeed, nothing errors. The tell is in the timestamps — consecutive runs whose start time equals the previous run's end time rather than the next tick. One case ran a heavyweight review loop this way around the clock and spent eighty-two percent of a day's entire model allowance on it. The second half of that failure is the part worth copying down: nobody found out from the system. There was no budget on the loop, no ceiling and no alarm, so the first real measurement of its cost was the human hitting a usage wall mid-afternoon and asking why — then hitting it again two days later. Two cheap controls for anything unattended that calls a model in a loop. Measure one real run and require the interval to exceed it with room to spare, or drop the fixed tick entirely for a self-paced loop that re-arms only when the previous pass has finished. And give it a daily budget it reports against, so the schedule has a cost you can see before it becomes a cost you can feel. One more thing, for the moment you find such a loop running away: that is a decision, not a status update. The line offering to kill it belongs in the first two sentences, not at the bottom of the analysis that found it.
...
**A second opinion from the same model is a mirror.** The critic-loop and second-opinion patterns assume the reviewer can fail differently from the author. Two instances of the same model, briefed identically, largely cannot — they share the training, the priors, and the same blind spots, so the second one tends to arrive at the first one's answer by the first one's route and hand you back a confidence boost you have not earned. Where the decision is genuinely load-bearing, put the question to a model from a *different vendor* and treat agreement across the two as the signal, not either verdict alone. A disagreement is at least as valuable: it locates the exact spot where the question is open rather than settled, and it is worth telling the user that it split rather than picking a winner. Three mechanics make it work in practice. Write the brief as facts, constraints, the artefact, then **numbered questions** — models answer numbered questions well and open-ended ones vaguely. Ask for **percentages** where you want a judgement you can actually compare between them. And say explicitly what to be blunt about, because the default register of every one of them is agreeable. Two traps sit in the plumbing rather than the prompt: a wrapper that reads a fixed input path instead of its argument will confidently review the wrong document forever, and a command-line tool that appends piped input to its prompt will hang indefinitely on an inherited non-interactive input stream. Keep every brief and every answer on disk under a timestamp, so a verdict can always be traced back to exactly what was asked.

**A second opinion is only as neutral as its brief.** Handing a second model the raw inputs rather than your conclusion is necessary and not sufficient, because whoever writes the brief is the first agent even when no verdict appears in it. Which heading a fact sits under, what it is placed next to, and which of the user's own stated reasons made it into the text all carry a judgement the reader inherits. One case put an action the user had explained, in so many words, as an attempt to make sense of a situation under a heading listing conduct he "had not denied", alongside facts that genuinely were adverse — and two independent models from two different vendors both assessed it as the adverse act the heading implied. Cross-vendor agreement on a slanted brief is agreement about the brief. When the subject is a person, or anything contested, keep headings descriptive rather than evaluative, put each stated reason next to the fact it explains, and have the person the brief is about — or a cold agent asked only "what does this framing assume?" — read it before it goes out.
...
**Two tools with the same name on the box, pointing at different screens.** Some capabilities exist twice in a setup — two ways to "open a browser", two ways to "send a message", two ways to "run a command on a machine" — and they differ only in *which machine they land on*. One drives a browser on a server; the other drives the browser on the desk in front of you. Both report success. Only one of them is visible to the person watching. If your standing instructions declare one of them "the" tool for the job, every session will reflexively reach for it, including in the case where it is exactly wrong — and the user will watch nothing happen on their screen while being told it's done. The correction is to write the *discriminator* into the instruction rather than the default: not "X is the browser", but "if the person is watching the result, use the one attached to their screen; if nobody is watching, use the headless one." The general form: when two tools do the same verb, the rule must name the condition that chooses between them, because a bare default is a coin-flip that always lands the same way. And the tell is unmistakable — if someone repeats the name of a machine at you, that is not emphasis, it is the parameter.
...
**Trusting a status field over the artifact.** Systems that do slow work usually expose a progress field — a state, a percentage, a "ready". It is tempting to treat that field as the answer to "is it done", because it is designed to look like exactly that. But a status field is a *claim the system makes about itself*, and it can be wrong in every direction at once: reporting nothing queued while a job sits in the queue, reporting a failure while the job is healthy and waiting, keeping a stale error long after a successful retry, and reporting complete before the output exists. Each of those wrong readings produces a different bad decision — abandoning a job that was fine, retrying one that was already running, declaring success before there is anything to hand over. The fix is a rule about evidence: **decide completion on the artifact, not the status.** Does the output have a real size, a real duration, does fetching it actually return bytes? Those cannot be wrong in the way a cached field can. Keep the status field for what it is genuinely good at — a rough progress indicator for a human watching — and never branch on it.

**An unwired tracking table reads as never-happened, always.** A system sometimes ships a table meant to record an external event — a lodgement, a sync, an approval — seeded and ready before the feature that would actually populate it exists. Querying it for "has this happened" returns empty, and empty looks exactly like a true negative: no rows, therefore no event. It never becomes a true negative, because nothing writes to it yet; the table is not stale, it is structurally silent, and every check against it says no forever, including the day the event genuinely happened through some other channel. A bookkeeping correction moved a transaction back into a quarter believed unfiled on the strength of an empty tracking table, and the person who had actually filed it said otherwise — he had lodged it and checked the figures himself first. The check is not whether the table looks stale; it is whether anything writes to it at all. When a proxy table's provenance is uncertain, verify against the primary record it is meant to shadow — the filed statement, the signed confirmation, the counterparty's own record — not the placeholder built to track it.

**Absence from the documented surface is not absence from the system.** When you need a capability and the reference doesn't list it, the honest conclusion is "not documented", not "not supported". The difference matters enormously, because the workaround for a missing capability is usually expensive and invasive — mutating a large number of objects, or rebuilding by hand something the system already does. Documentation lags implementation as a matter of course: a feature shipped last week is real in the code and absent from every guide. Before committing to a workaround whose blast radius is large, spend two minutes reading the actual interface — the source, the schema, the generated API description. The general rule scales with the cost of being wrong: the more objects your workaround would touch, the more obliged you are to prove the direct route doesn't exist. If you find yourself planning to modify dozens of things to achieve what one parameter would do, that is the signal to go and look.

**A probe that succeeds is not the call you are about to make.** Before working through an API you check that you can reach it, and the check is naturally the cheapest call available — a read, a health endpoint, a list. It comes back clean and access feels settled. But credentials carry *scopes*, and the scope that lets you read is routinely not the scope that lets you write. The failure then surfaces at the mutating call, after the plan is committed and several steps have already run, and it reads like an outage rather than what it is: a permission you never had. The same trap has a network variant — the address you probed from one machine is not the address the job will use from another, and a reachable endpoint on your desk can be unreachable from the box that runs the work. Probe with the operation you actually intend to perform, using the same credential, from the same place. Creating and deleting a throwaway object beats a successful read every time. Where rehearsing it genuinely is too destructive, read the credential's own scope list instead of inferring authorisation from the fact that a page loaded.

**Driving a browser reads whatever the page is holding.** Browser tooling is the right answer when a system has no API, and it has a property the other tiers don't: what comes back is not scoped to what you asked for. A script that lists the links on a page so it can find the one to click will return *every* link — and on an authenticated application, the navigation is the content. Point that at a logged-in messaging app and you get the conversation list with message previews; at a mail client, subject lines; at a bank, account names and balances. All of it lands in the transcript, which is a document that persists, syncs, and sometimes gets shared. The second half of the trap is that authentication state is a moving target. A profile you observed as logged out an hour ago may be logged in now — either because the person sat down at it, or because the page finished authenticating after you looked — so an assurance that "this profile is signed into nothing" has a shelf life measured in minutes. Two habits close it: scope the selector to the single element you need instead of enumerating the page, and re-check the authentication state immediately before acting on any profile a human also uses. Not at the start of the session, and never from a note.

**Reading a fact off a downsampled image.** Vision tooling makes images feel like just another readable surface, and mostly it is — but a thumbnail, a contact sheet, or a scaled-down screenshot has had the glyphs destroyed before the model ever sees it. What comes back is not a reading; it is a reconstruction, assembled from whatever plausible thing fits the shape. And it will be plausible: a real place name, a real person, a serial number in the right format, a figure with the right number of digits. That is exactly what makes it dangerous — it reads as knowledge rather than as a guess, so nothing about the answer invites checking. The damage compounds when the guess becomes a name: a folder, a file, a record, a deliverable titled after a fact that was never there, which then propagates as though it had been verified. The discipline is small and absolute. Before asserting anything read off an image — signage, labels, nameplates, screens, handwriting, a number on an invoice — crop that region and render it at full resolution, then read it again. If it still cannot be resolved, say so and name what is blocking; "unverified" is a perfectly good answer and a fabricated place name is not. Thumbnails are for triage — is this in focus, is this a duplicate, is this the right frame. They are not a source of text.

**Reading state off a copy that isn't the whole state.** To check whether something has happened — a device registered, a record written, a login landed — the assistant copies the store somewhere safe and queries the copy, which feels like the careful option. But some stores are not one file: a database in write-ahead mode keeps its newest writes in a sidecar log until a checkpoint, a replica lags its primary, a cache holds the previous page. The copy is internally consistent and minutes stale, and the query comes back empty — so the assistant reports "not done", six minutes after the user did it themselves. Asserting not-done from a broken probe is the same failure as asserting done from one; it just points the other way, and it is harder to catch because absence looks like the safe answer. Read live state through the thing that owns it — the application's own endpoint, the database engine against the real path — and if you must copy, copy the *whole* state: the export the engine provides, or every sidecar file together, never the main file alone.

**Naming an external actor before ruling out your own last action.** Investigating who or what caused an observed effect — a link opened, a record changed, an event logged — the assistant reasons from the effect back to the most plausible external cause and builds on it: an incident write-up, an accusatory draft, escalating language, all resting on a "who did this" the assistant never actually checked. The assistant's own tool calls in the same window are a candidate cause too, and the cheapest one to rule out — a test click, a rehearsal run, a probe made minutes earlier by the very tooling doing the investigating. Skip that check and the conclusion can be stated with total confidence and still be entirely wrong, and every escalation built on top of it inherits the error, until someone asks the one question that matters: was that us? Before naming an external actor as the cause of anything observed, grep your own tool-call log for the same window first — it is one command, and it belongs before the theory, not after the confrontation.

**Believing an unpaginated read is the whole answer.** A bulk read against a store of financial or otherwise consequential records — an API list endpoint, a database query — usually comes back with a page, not the set, and the API rarely refuses to answer when there is more; it just stops at its default page size and returns exactly as confidently as it would for a complete result. A reconciliation built on that page draws a real conclusion from a fake total: months of records outside the window read as "nothing happened", entries that already exist read as "missing", and the fix for a missing thing is to create it — so the gap becomes a duplicate that then has to be found and undone. The tell is a bug of this shape recurring twice in one sitting: that is not two unrelated defects, it is one habit not yet corrected. Before any conclusion drawn from a bulk read is allowed to drive a write — a posting, a match, an "unmatched" claim — check that the read is complete: compare the count returned against the total the endpoint reports, or page until an empty page comes back. Never accept a raw row count as "probably everything" merely because it looks like a plausible amount.

**A credential that fails is not necessarily a dead credential.** A call comes back 401 or 403 and the conclusion writes itself: the token has expired, mint a new one. That conclusion is wrong often enough to be worth a rule, because the same status codes are used for conditions that have nothing to do with expiry — a permission scope that was never granted, a resource the credential does not cover, and in particular an *origin restriction*, where the token is perfectly valid but refuses to work from the address you happen to be calling from. That last one is invisible until you read the error body: the machine-readable code inside it names the actual condition, and it is the only part of the response that distinguishes "expired" from "not from here". Rotating a credential on a bad diagnosis is not free — it invalidates the working copy everywhere else it is used, and it leaves the real fault in place for the next caller to rediscover. Read `errors[].code`, or whatever the equivalent field is called, before declaring anything dead. A verify endpoint that reports the credential active while every real call fails is the signature of a scope or origin condition, never of expiry.

**A wrapper that falls back to a stronger credential hides its own failure.** The reason a broken credential can sit unnoticed for months is usually a helper somebody wrote to be robust. It tries the narrow, purpose-scoped credential, and if that fails it quietly retries with the account-wide one — and since the operation then succeeds, nothing ever surfaces. Two things are true at once afterwards: the tool works, and the scoped credential has been useless since some unknown date. The consequence is not theoretical. Every consumer that can *only* take the narrow credential — a certificate client, an infrastructure provisioner, anything without the fallback baked in — fails in a way that looks unrelated, while the setup as a whole appears healthy. And the work is being done, continuously, by the most dangerous key you own. Fallbacks between credentials of different power need to be loud: log which one was used, or refuse to fall back at all and fail with a message naming the scoped credential. A silent downgrade to a stronger key is the same class of defect as a retry that swallows an exception.

**Reaching for discovery before the source that already has a name.** When a specific, already-documented live source exists for a question -- a project database, a service API, a config file — the correct first tool call is that source, not a general investigation. Memory search, transcript grep, network probes, and web scraping are each reasonable in isolation, but none of them is aimed at where the answer actually lives, and running them first can burn dozens of calls without ever producing an answer to a question that had one obvious owner from the first message. The tell is a long chain of calls that never converges, on something a single documented query would settle in one call. Check whether the domain already has a named source before opening a general investigation, and only widen the search once that source has actually failed to answer.

**A killed run leaves a clean log.** Judging a long command by searching its output for failure markers — grep the test log for `FAILED`, look for `ERROR`, count the red lines — reads as diligence, and it inverts exactly where it matters most. A run that is killed partway through, whether by a timeout in the code that was waiting on it or by the machine reclaiming memory from it, stops writing at the moment it dies. It never reaches the failures further down and it never prints its summary, so the log it leaves behind contains no failure markers at all and the search comes back clean. A suite terminated at two-thirds and a suite that passed give the same answer to `grep FAILED`; they differ only in the exit status, which is the thing nobody looked at. Gate on the exit code, and treat the log as evidence about *what* failed, never as evidence *that* nothing did — and treat a suspiciously tidy log from a long job as a reason to go and check how it ended. A companion cause, because it produces this exact signature: starting a second heavy job alongside one already running, on a machine with room for either but not both. The kernel picks one and removes it, and what comes back is not an error about resources — it is a truncated, entirely reasonable-looking log.

**A filter the system silently ignores.** Parameters that narrow a result — a user id, a date range, a status — get trusted implicitly, on the reasonable assumption that an unsupported filter would be an error. Often it isn't: the parameter is simply never read, the unfiltered set comes back, and it is indistinguishable from a filtered set that happened to contain those items. Everything built on top is then a claim about the wrong population, made with the confidence of a targeted query. One case listed authentication devices "for user seven" and drew per-user conclusions from the four records returned, discovering only later that user twenty-three — and a nonsense user nine hundred and ninety-nine — returned the same four. The conclusions had been right, because all four devices happened to belong to that user anyway; that is luck, not method. The check costs one call: ask the filter for something that *must* come back empty. If a value you know is absent still returns a full set, the parameter is inert. The harder version of the same discipline is using a search to establish that something does *not* exist. A relevance-ranked search returning nothing is not proof of absence — it is a ranking function declining to surface anything, and it looks exactly like a genuinely empty set. When the conclusion is a negative — no message was ever sent, no record exists in that window — enumerate the set and count it. One case asserted a missing notification on the strength of a keyword search and then had to re-establish it properly by listing all seven hundred and twenty-seven items in the window. The answer held, but it had already been stated on evidence that could not carry it.

**A wholesale rewrite of a document under revision drops what the rewrite didn't remember to keep.** When a document has been through many rounds of small, spoken corrections — a message, a report, a config file assembled line by line — the natural response to "here's another list of changes" is to regenerate the whole body from your current understanding of what it should say. That regeneration is built from your summary of the document, not from the document, and anything the summary quietly dropped goes with it: a paragraph the requester deliberately kept in through three earlier rounds simply isn't there any more, and nothing about the new version looks wrong on its own — it reads fine, it just isn't complete. The same session can compound this from the other direction: a tool call that is rejected or interrupted partway through is not the same thing as one that never ran, and treating it as a clean no-op means the next edit lands on a file that already changed, producing duplicated content instead of missing content. Both failures share one fix. On a document under active, multi-round revision, apply each requested change as a targeted edit against the current file, never a full regenerate of the body — and after any tool call whose outcome is ambiguous, re-read the file before trusting your model of what it contains.

**Undoing a change you assumed your own tooling made.** The mirror image of naming an external actor: an unexplained change lands inside the window of a workflow you are running — records deleted, files moved, a setting flipped — and the timing makes it look like one of your own agents went off-script, so you put it back. One case restored well over a hundred gigabytes and re-added four entries to a media library that the user had deleted, deliberately, a few minutes earlier; he then had to ask for the same deletion twice. Overlap in time is not attribution. Before reverting anything you did not knowingly do, tie the change to an actor — the user or key recorded in the application's own log, the source address, or the exact calls in your tool-call log — and if it cannot be tied to your own calls, ask. A revert of someone else's deliberate action is not a fix; it is a second destructive change, made with better intentions.

**Measuring exposure from inside the fence.** A test of whether something is reachable from the internet, run from a machine inside the same network, measures the network's view of itself. Hairpin NAT and allow-listed local ranges happily answer a request aimed at the public address, so a service looks directly exposed when no outside packet can reach it at all. One case reported a service as bypassable straight to its public address, added a redundant allow-list layer to close the hole, and only then repeated the test from a genuinely external host — the port had been restricted to the CDN's ranges for months, and the new layer came straight back out. The vantage point is part of the measurement: test exposure from outside, before acting on the result. The same discipline at the other end of the stack — when the user says the page is broken, load the page they are loading. Healthy endpoints and a broken page coexist without any difficulty, and reading the APIs behind it answers a different question from the one asked.

**Extracted text is not the rendered document.** When the deliverable is a document that leaves the building — an invoice, a quote, a submission, a report — the natural verification is to pull the text out of it and search for what should and shouldn't be there. It passes, and it keeps passing, because text extraction discards the two things most likely to be wrong: where content landed, and how many times it appears. One invoice was verified three times that way and reported correct three times, while the rendered page carried an internal cost-and-markup note on the customer copy, a near-empty second page holding nothing but duplicated payment boilerplate, and one clause printed twice in a single paragraph. Rendering the pages to images and looking at them found all three in seconds — including the page a text search had happily reported as containing no matches. Render and read the pages, and check the page count as well as the content; a text search is a good *second* pass for a specific leak, and a bad first pass for "does this look right".

**A bot wall is a verdict on the client, not on you.** A page that returns 403 with a challenge or a "we detected a problem with your browser" message has almost never blocked your address — it has declined your *client*, on headers, TLS fingerprint and the absence of everything a real browser does. The same page on the same machine, loaded by a headless browser with a real profile, returns 200. Reading the refusal as an IP ban sends the work down an expensive wrong path: proxies, waiting, or telling the user the site is unreachable when it is one tool-tier away. Move up a tier — command-line fetch, then a real browser engine, then a browser with a human's own profile — and treat the error text as a description of the client that was rejected. Two practical notes. Pin the browser executable explicitly if the installed bundle and the library's expected version differ, or the launch fails in a way that reads like an outage. And know where the ceiling is: the strongest commercial anti-bot systems are not beaten by a headless browser either, and grinding at one of those is the failure mode this rule is supposed to prevent, not license for it.

**An archive records what people wrote down.** Searching a corpus — a message history, a mailbox, a ticket system, a document store — is excellent for dates, sequence, and exact wording, and close to useless for establishing that something did not happen. The things that matter most are routinely the things nobody types: a fight, a diagnosis, an admission made out loud, a decision taken in a room. A clean sweep of every message one person ever wrote returned nothing on a subject they had described verbally, and the empty result was reported as a finding about reality rather than about the corpus; two further readings were built on it before both had to be withdrawn. Report a sweep as "not recorded in X", with the corpus named and its limits stated, and before concluding from silence ask whether the thing is one anybody would ever put in writing. And when the person you are working for states something from their own memory, that outranks the absence of text — write it down as their account and go back and correct whatever the silence had already been used to support.
