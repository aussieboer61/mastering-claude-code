...
|------|-----------|----------|------------------|------|
| 08 | 2026-07-06 | **A rich store the assistant never opens** — the quietest memory failure: everything is recorded and a parallel system gets built from scratch anyway, because the index was never read before starting. New Ch 2 failure mode + standing default: read the index at build-start, design the smallest delta on what exists. Plus: **secret hygiene as a hook pair** (PreToolUse deny on printing secret files + PostToolUse output scan for credential shapes, with a sanctioned redacting reader) in Ch 4; three Ch 1 behavioural defaults (**sanity-check results against known context**, **ask why the primary path is out first**, **the target is the deliverable**); and a Ch 9 failure mode: **adopt a maintained tool over commissioning a bespoke dashboard** — bespoke status surfaces rot. | 1 (CLAUDE.md), 2 (Memory), 4 (Hooks), 9 (Tool discipline) | [week-08.md](week-08.md) |
