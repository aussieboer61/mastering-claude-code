...
|------|-----------|----------|------------------|------|
| 10 | 2026-07-20 | **The quiet failures are the ones that report success.** Five new failure modes, all from systems that looked healthy while losing something. Ch 4: **hook registrations live in a file, and ordinary file operations can revert them** — a version-control rewind resurrects retired hooks noisily and destroys added ones in total silence; auto-commit the settings file so there's no gap to rewind into. Ch 6: **reconcile ran after collect, and the gap was the data** — a nightly job that resets to the remote *after* gathering its snapshot discards the work and logs "nothing to commit"; a path that can discard work must never exit successfully with a benign message. Ch 5: **an agent asked to clean up, deciding what "dead" means** — broken is not unused, and low activity is a symptom, not a verdict; cleanup produces a list, not a decision, and means *move with a manifest*, never delete. Ch 9: **two tools with the same name, pointing at different screens** — write the discriminator into the rule, not the default; a repeated machine name is a parameter, not emphasis. Ch 2: **the store as a target the moment anything serves it** — authenticate the reader in version one, and keep secret values out of the store. Two Ch 1 defaults: **a settled decision outranks a fresh derivation** and **a scope mentioned in a status update is not consent**. | 1 (CLAUDE.md), 2 (Memory), 4 (Hooks), 5 (Subagents), 6 (Persistence), 9 (Tool discipline) | [week-10.md](week-10.md) |
